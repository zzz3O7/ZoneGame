...
- Пропуск глобального хода → **−10% от текущих очков игрока в момент пропуска** (минимум 0, округление на усмотрение балансировки).
...
Партия заканчивается когда оба игрока пропустили глобальный ход подряд. В этот момент все зоны уже завершены — незавершённых зон не существует. Доказательство: если оба пропустили, значит в каждой зоне тот, чей локальный ход, не мог сделать ход — то есть он уже проиграл эту зону. Победитель партии — игрок с большей суммой очков.
